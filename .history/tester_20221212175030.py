...
""" Returns the next available filename given a prefix and extension (does not return string with extension"""
def get_next_name(path_prefix: str, ext: str) -> str:
    ...
    for f in sorted(os.listdir()):
        ...
        if re.match(prefix_pattern, root) and re.match(ext_pattern, f_ext):
            f_num = int(re.match(prefix_pattern, root).group(1).lstrip('0'))
            print(f"FNUM: {f_num}")
            i = f_num+1
            
    os.chdir("..")
    return f"{path_prefix}-{str(i).zfill(4)}"

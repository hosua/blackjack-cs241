...
""" Returns the next available filename for the prefix"""
def next_path(path_prefix: str, ext: str) -> str:
    i: int
    os.chdir(SAVE_DIR)
    matches = []
    prefix_pattern = f"^{path_prefix}.*"
    ext_pattern = f".*{ext}$"
    extract_number_pattern = "T\d+-\d"
    for f in os.listdir():
        root, f_ext = os.path.splitext(os.path.expanduser(f))
        fname = os.path.basename(root)
        print(f"{root} {ext}")
        if re.match(pattern, root) and re.match(ext_pattern, f_ext):
            matches.append(fname)
            re.match(extract_number_pattern, fname)
        # if root.startswith(path_prefix) and ext == f_ext:
    matches = sorted(matches)
    print(matches)
    os.chdir("..")
    return f"{path_prefix}-{i}.{ext}"
...
def dump_json(stats_dict: dict, path_prefix: str):
    ...
    output_fname = f"{SAVE_DIR}/{path_prefix}.json"
